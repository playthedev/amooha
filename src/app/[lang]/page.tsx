...
import {
  ArrowRight, BadgeCheck, BriefcaseBusiness, Leaf, Newspaper,
  RefreshCcw, Sprout, Users, Wheat, MapPin, FlaskConical,
  Truck, ShieldCheck, ChevronRight, Star, Quote, TrendingUp, Package, Globe,
} from "lucide-react";
...
export default async function HomePage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const leadStory = d.heroStories[0];
  const secondStory = d.heroStories[1];
  ...
  return (
    <main className="bg-[var(--cream)] text-[var(--ink)]">
      ...
      <section className="relative min-h-[calc(100vh-127px)] overflow-hidden bg-[var(--leaf-dark)] text-white">
        <Image src={heroImages[0]} alt="Amoohaa Farms field" fill priority sizes="100vw" className="object-cover opacity-60" />
        ...
        <div className="section-shell relative z-10 mx-auto flex min-h-[calc(100vh-127px)] max-w-7xl flex-col justify-end pb-24 pt-28 px-5 sm:px-8 lg:px-10">
          <div className="max-w-4xl">
            ...
            <h1 className="display-serif mt-6 text-6xl font-normal leading-[1.05] sm:text-7xl lg:text-[5.5rem]">{leadStory.title}</h1>
            <p className="mt-7 max-w-xl text-lg leading-8 text-white/80">{leadStory.body}</p>
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              <Link href={`/${lang}/introduction`} className="inline-flex h-14 items-center justify-center gap-3 bg-[var(--wheat)] px-9 text-[11px] font-extrabold uppercase tracking-[0.2em] text-[var(--leaf-dark)] transition hover:brightness-105">
                {dict.home.community.cta} <ArrowRight size={16} />
              </Link>
              <Link href={`/${lang}/brands`} className="inline-flex h-14 items-center justify-center border border-white/30 px-9 text-[11px] font-extrabold uppercase tracking-[0.2em] text-white transition hover:bg-white/10 hover:border-white/60">
                {dict.nav.brands}
              </Link>
            </div>
          </div>
        ...
        <div className="absolute bottom-8 right-8 hidden w-72 border border-white/15 bg-white/5 p-5 backdrop-blur-sm md:block">
          ...
          <p className="mt-3 text-[15px] font-semibold leading-snug">{secondStory.title}</p>
          <p className="mt-2 text-[12px] text-white/55 leading-relaxed">{secondStory.body}</p>
        </div>
      </section>
      ...
      <section className="bg-[var(--wheat)] py-16 px-5 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
            {d.numbers.items.map(({ value, label }, i) => {
          ...
          <div className="mb-14 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <div className="flex items-center gap-3">
                ...
                <span className="label-caps text-[var(--wheat-deep)] tracking-[0.22em]">{d.news.label}</span>
              </div>
              <h2 className="display-serif mt-3 text-4xl font-normal sm:text-5xl">Latest from Amoohaa</h2>
            </div>
          </div>
          <div className="grid gap-6 md:grid-cols-3">
            {d.news.cards.map((card, idx) => (
              <article key={card.title} className="group relative flex flex-col border border-[var(--line)] bg-[var(--cream)] p-8 transition-all duration-300 hover:border-[var(--wheat)] hover:shadow-lg hover:shadow-green-900/6 hover:-translate-y-0.5">
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-16 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="flex items-center gap-3">
                ...
                <span className="label-caps text-[var(--wheat)] tracking-[0.22em]">{d.journey.label}</span>
              </div>
              <h2 className="display-serif mt-3 text-4xl font-normal sm:text-5xl">{d.journey.title}</h2>
            </div>
          </div>
          <div className="grid gap-px bg-white/8 sm:grid-cols-2 lg:grid-cols-4">
            {d.journey.steps.map((step, i) => {
              ...
              return (
                <div key={step.num} className="group relative bg-[var(--leaf-dark)] p-8 pb-10 transition-colors hover:bg-white/5">
                  <div className="mb-8 flex items-start justify-between">
                    ...
                    <span className="text-[11px] font-bold text-white/18 tracking-[0.16em]">{step.num}</span>
                  </div>
                  <h3 className="text-[13px] font-extrabold uppercase tracking-[0.14em] text-white">{step.title}</h3>
                  <p className="mt-3 text-[13px] leading-7 text-white/55">{step.body}</p>
          ...
          <div className="mt-12 flex flex-col items-start gap-4 border-t border-white/8 pt-10 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link href={`/${lang}/contact`} className="inline-flex shrink-0 h-12 items-center gap-3 border border-[var(--wheat)]/35 px-7 text-[11px] font-extrabold uppercase tracking-[0.2em] text-[var(--wheat)] transition hover:bg-[var(--wheat)]/10">
              {dict.nav.contact} <ArrowRight size={14} />
            </Link>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="grid grid-cols-1 border border-[var(--line)] bg-[var(--paper)] sm:grid-cols-3">
            {numbers.map(([value, label], i) => (
              <div key={label} className={`flex min-h-36 flex-col justify-between gap-8 p-6 sm:min-h-52 sm:p-8 ${i < 2 ? "border-b border-[var(--line)] sm:border-b-0 sm:border-r" : ""}`}>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-14 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <div className="flex items-center gap-3">
                <span className="h-px w-6 bg-[var(--wheat-deep)]" />
                <span className="label-caps text-[var(--wheat-deep)] tracking-[0.22em]">{d.categories.label}</span>
              </div>
              <h2 className="display-serif mt-3 text-4xl font-normal sm:text-5xl">{d.categories.title}</h2>
            </div>
            <Link href={`/${lang}/products`} className="inline-flex items-center gap-2 text-[11px] font-extrabold uppercase tracking-[0.2em] text-[var(--leaf-dark)] transition hover:text-[var(--leaf)]">
              {d.categories.cta} <ArrowRight size={14} />
            </Link>
          </div>
          <div className="grid gap-5 sm:grid-cols-3">
            {productCategories.map((cat, i) => {
              const label = d.categories.items[i]?.label ?? "";
              return (
                <div key={i} className="group relative overflow-hidden" style={{ minHeight: 380 }}>
                  <Image src={cat.image} alt={label} fill sizes="(max-width: 640px) 100vw, 33vw" className="object-cover transition duration-700 group-hover:scale-105" />
                  <div className="absolute inset-0 transition-opacity duration-300 group-hover:opacity-95" style={{ background: cat.gradient }} />
                  <div className="absolute inset-0 flex flex-col justify-between p-8">
                    <span className="self-start border border-[var(--wheat)]/40 bg-black/20 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.2em] text-[var(--wheat)] backdrop-blur-sm">Power Pulz</span>
                    <div>
                      <h3 className="display-serif text-3xl font-normal text-white leading-tight">{label}</h3>
                      <div className="mt-5 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.18em] text-[var(--wheat)] opacity-0 -translate-y-1 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0">
                        <Link href={`/${lang}/products`}><span>{d.categories.cta}</span></Link>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-14 grid gap-8 lg:grid-cols-12">
            <div className="lg:col-span-4 lg:pr-10">
              <div className="flex items-center gap-3">
                <span className="h-px w-6 bg-[var(--wheat-deep)]" />
                <span className="label-caps text-[var(--wheat-deep)] tracking-[0.22em]">{d.community.label}</span>
              </div>
              <h2 className="display-serif mt-3 text-4xl font-normal leading-tight sm:text-5xl">{d.community.title}</h2>
              <p className="mt-6 text-[15px] leading-8 text-[var(--stone)]">{d.community.body}</p>
            </div>
            <div className="grid gap-5 sm:grid-cols-3 lg:col-span-8">
              {[
                ...
                <article key={title as string} className="group relative flex flex-col border border-[var(--line)] bg-[var(--paper)] p-8 transition-all duration-300 hover:border-[var(--leaf)]/40 hover:bg-white hover:shadow-md">
                  <div className="mb-auto flex h-11 w-11 items-center justify-center border border-[var(--wheat)]/30 bg-[var(--wheat)]/8 text-[var(--wheat-deep)]">
                    <Icon size={20} />
                  </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-16 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="flex items-center gap-3">
                <span className="h-px w-6 bg-[var(--wheat-deep)]" />
                <span className="label-caps text-[var(--wheat-deep)] tracking-[0.22em]">{d.testimonials.label}</span>
              </div>
              <h2 className="display-serif mt-3 text-4xl font-normal sm:text-5xl">Trusted by those who source with care</h2>
            </div>
            <div className="flex items-center gap-3 self-start border border-[var(--line)] bg-[var(--cream)] px-5 py-4">
              <div className="flex gap-0.5">
                {[...Array(5)].map((_, i) => <Star key={i} size={13} className="fill-[var(--wheat)] text-[var(--wheat)]" />)}
              </div>
              <div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {d.testimonials.items.map((t, idx) => (
              <div key={t.name} className={`flex flex-col border p-8 transition-all duration-300 ${idx === 1 ? "border-[var(--wheat)] bg-[var(--leaf-dark)] text-white" : "border-[var(--line)] bg-[var(--cream)] hover:border-[var(--wheat)]/50 hover:-translate-y-0.5 hover:shadow-md"}`}>
                <Quote size={26} className={`mb-6 ${idx === 1 ? "text-[var(--wheat)]/50" : "text-[var(--wheat-deep)]/35"}`} />
                <p className={`flex-1 text-[14px] leading-8 ${idx === 1 ? "text-white/80" : "text-[var(--stone)]"}`}>&ldquo;{t.quote}&rdquo;</p>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-14 flex items-end justify-between gap-6">
            <div>
              <div className="flex items-center gap-3">
                <span className="h-px w-6 bg-[var(--wheat-deep)]" />
                <span className="label-caps text-[var(--wheat-deep)] tracking-[0.22em]">{d.brands.label}</span>
              </div>
              <h2 className="display-serif mt-3 text-4xl font-normal sm:text-5xl">{d.brands.title}</h2>
            </div>
            <Link href={`/${lang}/brands`} className="hidden items-center gap-3 text-[11px] font-extrabold uppercase tracking-[0.2em] text-[var(--leaf-dark)] transition hover:text-[var(--leaf)] sm:inline-flex">
              View all <ArrowRight size={15} />
            </Link>
          </div>
          <div className="grid gap-6 sm:grid-cols-2">
            {d.brands.items.map((brand, i) => {
              const bc = brandCards[i];
              return (
                <a key={brand.name} href={bc.href} target={bc.href.startsWith("http") ? "_blank" : undefined} rel={bc.href.startsWith("http") ? "noreferrer" : undefined}
                  className="group relative min-h-[480px] w-full overflow-hidden bg-[var(--leaf-dark)] text-white">
                  <Image src={bc.image} alt={brand.name} fill sizes="(max-width: 640px) 100vw, 50vw" className="object-cover transition duration-700 group-hover:scale-105" />
                  ...
                  <div className="absolute inset-x-0 bottom-0 flex flex-col p-10">
                    <div className="flex items-center gap-2" style={{ color: bc.accentColor }}>
                      <span className="h-px w-5" style={{ background: bc.accentColor }} />
                      <p className="label-caps text-[10px] tracking-[0.22em]">{brand.name}</p>
                    </div>
                    <h3 className="display-serif mt-3 text-4xl font-normal">{brand.name}</h3>
                    <p className="mt-2 text-[13px] text-white/65 leading-relaxed">{brand.desc}</p>
                    <div className="mt-6 flex items-center gap-3 text-[11px] font-bold uppercase tracking-[0.18em] text-white/70">
                      <span>{brand.cta}</span>
      ...
      <section className="bg-[var(--leaf-dark)] py-14 px-5 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            {[[BadgeCheck, "FSSAI aligned"], [BriefcaseBusiness, "B2B ready"], [Wheat, "Farm origin"], [Leaf, "Sustainable growth"]].map(([Icon, label]) => (
              <div key={label as string} className="group flex flex-col items-center justify-center gap-4 border border-white/10 bg-white/[0.03] py-8 px-6 text-center transition-colors hover:border-[var(--wheat)]/30 hover:bg-white/[0.06]">
                <Icon size={22} className="text-[var(--wheat)]" />
                <span className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-white/60 group-hover:text-[var(--wheat)]">{label as string}</span>
              </div>
            ))}
          </div>
        </div>
